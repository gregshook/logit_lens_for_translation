...
def construct_few_shot_prompts(
    ...
    for sent in source_prompts[:num_examples]:
        if num_shots > 0:
            ...
            if num_shots == 1:
                prompt = f"Translate. Example: {examples_text} {source_lang}: {sent}. {target_lang}:"
            else:
                prompt = f"Translate. Examples: {examples_text} Now: {source_lang}: {sent}. {target_lang}:"
        else:
            # Zero-shot case
            prompt = f"Translate from {source_lang} to {target_lang}: {source_lang}: {sent}. {target_lang}:"
...
if __name__ == "__main__":
    ...
    languages = ["english", "german", "czech", "arabic"]
    csv_files = {
        "english": "english_flores_prompts.csv",
        "german": "german_flores_prompts(1).csv",
        "czech": "czech_flores_prompts(1).csv",
        "arabic": "arabic_flores_prompts(1).csv"
    }
